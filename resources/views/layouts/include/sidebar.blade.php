<div class="sidebar sidebar-desktop d-none d-lg-block bg-white">
    <nav class="nav nav-pills flex-column">
        ...
        @if (hasPermission('sale_orders'))
        <a class="flex-sm-fill text-start nav-link mb-2 {{ in_array(request()->segment(2), ['sales-order', 'sales-order-details', 'route-card-details']) ? 'active' : '' }}"
            ...
            </span> <span class="">Sale Orders</span>
        </a>
        @endif

        @if (hasPermission('operations'))
        <a class="flex-sm-fill text-start nav-link mb-2 {{ in_array(request()->segment(2), ['operations']) ? 'active' : '' }}"
            aria-current="page" href="{{ route('admin.operations') }}">
            <span class="sidebar-icon">
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="20" viewBox="0 0 16 18" fill="none">
                    <path
                        d="M3 17.3332C2.30556 17.3332 1.71528 17.0901 1.22917 16.604C0.743056 16.1179 0.5 15.5276 0.5 14.8332V12.3332H3V0.666504L4.25 1.9165L5.5 0.666504L6.75 1.9165L8 0.666504L9.25 1.9165L10.5 0.666504L11.75 1.9165L13 0.666504L14.25 1.9165L15.5 0.666504V14.8332C15.5 15.5276 15.2569 16.1179 14.7708 16.604C14.2847 17.0901 13.6944 17.3332 13 17.3332H3ZM13 15.6665C13.2361 15.6665 13.434 15.5866 13.5938 15.4269C13.7535 15.2672 13.8333 15.0693 13.8333 14.8332V3.1665H4.66667V12.3332H12.1667V14.8332C12.1667 15.0693 12.2465 15.2672 12.4062 15.4269C12.566 15.5866 12.7639 15.6665 13 15.6665ZM5.5 6.49984V4.83317H10.5V6.49984H5.5ZM5.5 8.99984V7.33317H10.5V8.99984H5.5ZM12.1667 6.49984C11.9306 6.49984 11.7326 6.41998 11.5729 6.26025C11.4132 6.10053 11.3333 5.90262 11.3333 5.6665C11.3333 5.43039 11.4132 5.23248 11.5729 5.07275C11.7326 4.91303 11.9306 4.83317 12.1667 4.83317C12.4028 4.83317 12.6007 4.91303 12.7604 5.07275C12.9201 5.23248 13 5.43039 13 5.6665C13 5.90262 12.9201 6.10053 12.7604 6.26025C12.6007 6.41998 12.4028 6.49984 12.1667 6.49984ZM12.1667 8.99984C11.9306 8.99984 11.7326 8.91998 11.5729 8.76025C11.4132 8.60053 11.3333 8.40261 11.3333 8.1665C11.3333 7.93039 11.4132 7.73248 11.5729 7.57275C11.7326 7.41303 11.9306 7.33317 12.1667 7.33317C12.4028 7.33317 12.6007 7.41303 12.7604 7.57275C12.9201 7.73248 13 7.93039 13 8.1665C13 8.40261 12.9201 8.60053 12.7604 8.76025C12.6007 8.91998 12.4028 8.99984 12.1667 8.99984Z" />
        ...
        @endif
        @if (hasPermission('machines'))
        <a class="flex-sm-fill text-start nav-link mb-2 {{ in_array(request()->segment(2), ['machines', 'machine-details']) ? 'active' : '' }}"
        ...
        @endif
        @if (hasPermission('operators'))
        <a class="flex-sm-fill text-start nav-link mb-2 {{ in_array(request()->segment(2), ['operators', 'operator-details']) ? 'active' : '' }}"
        ...
        <div class="nav-item dropdown mb-2 {{ in_array(request()->segment(2), ['production-overview', 'maintenance-overview', 'so-completion-tracking', 'so-roll-tracking', 'maintenance-history']) ? 'active' : '' }}">
            ...
            <ul class="dropdown-menu {{ in_array(request()->segment(2), ['production-overview', 'maintenance-overview', 'so-completion-tracking', 'so-roll-tracking', 'maintenance-history']) ? 'active show' : '' }}" aria-labelledby="reportsDropdown">
                ...
                <li>
                    <a class="dropdown-item {{ in_array(request()->segment(2), ['maintenance-overview']) ? 'active' : '' }}" href="{{ route('admin.maintenance-overview') }}">
                        MIS-Maintenance Overview
                    </a>
                </li>
                <li>
                    <a class="dropdown-item {{ in_array(request()->segment(2), ['so-completion-tracking']) ? 'active' : '' }}" href="{{ route('admin.so-completion-tracking') }}">
                        SO-Completion Tracking
                    </a>
                </li>
                <li>
                    <a class="dropdown-item {{ in_array(request()->segment(2), ['so-roll-tracking']) ? 'active' : '' }}" href="{{ route('admin.so-roll-tracking') }}">
                        SO-Roll Tracking
                    </a>
                </li>
                <li>
                    <a class="dropdown-item {{ in_array(request()->segment(2), ['maintenance-history']) ? 'active' : '' }}" href="{{ route('admin.maintenance-history') }}">
                        Maintenance History
                    </a>
                </li>
            </ul>
        ...
        @endif
        @if (hasPermission('route_cards'))
        <a class="flex-sm-fill text-start nav-link mb-2 {{ in_array(request()->segment(2), ['route-cards', 'process']) ? 'active' : '' }}"
