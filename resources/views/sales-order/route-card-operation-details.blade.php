                                        ...
                                        @if (!empty($getOperationData) && count($getOperationData) > 0)
                                            @foreach ($getOperationData as $operation)
                                                ...
                                                @php
                                                    ...
                                                    // Calculate the difference (positive = exceeded, negative = faster)
                                                    $gapInSeconds = $totalTimeForRoll - $idealTimeInSeconds;

                                                    // Determine sign
                                                    $sign = $gapInSeconds > 0 ? '+' : ($gapInSeconds < 0 ? '-' : '');

                                                    // Convert to absolute for formatting
                                                    $absGap = abs($gapInSeconds);
                                                    $hours = floor($absGap / 3600);
                                                    $minutes = floor(($absGap % 3600) / 60);
                                                    $seconds = $absGap % 60;

                                                    // Format as hh:mm:ss
                                                    $formattedGap = sprintf(
                                                        '%s%02d:%02d:%02d',
                                                        $sign,
                                                        $hours,
                                                        $minutes,
                                                ...
                                                <tr>
                                                    <td class="align-center p-3"
                                                        ...
                                                        <h2 class="accordion-header d-flex justify-content-around">
                                                            <span
                                                                class="fs-6 fw-semibold text-445B64">#{{ $operation->quantity_processed ?? '1' }}</span>
                                                        </h2>
                                                    </td>
